...
public class ComputerDao {
  ...
  public void checkData(Computer computer) {
    if (computer.getName() == null || computer.getName().trim().isEmpty()) {
      throw new ComputerNullNameException("Failed to create computer : Computer name is null.");
    }

    if (!computer.getIntroduced().isPresent() && computer.getDiscontinued().isPresent()) {
      throw new DiscontinuedButNoIntroducedException("Failed to create computer : Dicontinued not null whereas introcued is.");
    }

    if ( computer.getIntroduced().isPresent() && computer.getDiscontinued().isPresent()
        && !computer.getDiscontinued().get().after(computer.getIntroduced().get())){
      throw new DiscontinuedBeforeIntroducedException("Failed to create computer : Discontinuation date is not after the introduction date.");
    }
  }
